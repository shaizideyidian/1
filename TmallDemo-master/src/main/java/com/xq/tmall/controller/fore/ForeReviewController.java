...
//        通过调用productService.get通过订单项的产品ID获取产品信息
        Product product = productService.get(orderItem.getProductOrderItem_product().getProduct_id());
//        通过reviewService.getTotalByProductId获取指定产品ID的评论总数
        product.setProduct_review_count(reviewService.getTotalByProductId(product.getProduct_id()));
//        通过productImageService.getList获取指定产品ID的单个产品图片列表，并将其设置为产品的图片列表，0表示获取类型为0的图片，new PageUtil表示返回第一页的数据，每页一个图片
...
//        通过reviewService.getListByProductId获取指定产品ID的评论列表，new PageUtil是一个分页工具，
        List<Review> reviewList = reviewService.getListByProductId(product_id, new PageUtil(index, 10));
        if (reviewList != null) {
            for (Review review : reviewList) {
//                通过评论对象中的用户ID获取用户信息，并将用户信息设置回评论对象中。
