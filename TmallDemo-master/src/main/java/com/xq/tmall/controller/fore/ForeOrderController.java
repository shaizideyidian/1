...
//        创建一个PageUtil对象，传入index和count作为参数，用于分页查询
        PageUtil pageUtil = new PageUtil(index, count);
        logger.info("根据用户ID:{}获取订单列表", userId);
//        调用productOrderService.getList方法，根据给定的查询条件，排序规则和分页参数，获取订单列表，查询条件包括用户ID和订单状态status_array
...
//        判断是否有订单数据，productOrderService.getTotal方法统计满足查询条件的订单总数
...
//            遍历productOrderList中的每个订单，通过productOrderItemService.getListByOrderId方法获取订单项列表，并依次获取每个订单项的产品信息
...
//        调用categoryService.getList方法，获取产品分类列表
...
//        categoryService是一个用于处理产品类别的服务，productImageService是一个用于处理产品图片的服务
        product.setProduct_category(categoryService.get(product.getProduct_category().getCategory_id()));
//        getList方法用于根据指定的条件获取产品图片列表，PageUtil是一个用于分页的实用类
        product.setSingleProductImageList(productImageService.getList(product_id, (byte) 0, new PageUtil(0, 1)));

        logger.info("封装订单项对象");
//        创建一个ProductOrderItem的对象
...
//        用于检查订单项列表中的订单项是否与指定的用户和订单匹配
        for (ProductOrderItem orderItem : orderItemList) {
//            orderItem.getProductOrderItem_user获取订单项所属的用户对象，并通过getUser_id获取用户ID
            if (orderItem.getProductOrderItem_user().getUser_id() != userId) {
                logger.warn("用户订单项与用户不匹配，回到购物车页");
                return "redirect:/cart";
            }
//            通过orderItem.getProductOrderItem_order获取订单项所属的订单对象
...
//        声明变量orderTotalPrice，初始值为0.0
        double orderTotalPrice = 0.0;
//        循环遍历订单项列表orderItemList中的每一个订单项。
        for (ProductOrderItem orderItem : orderItemList) {
//            通过orderItem.getProductOrderItem_product().getProduct_id获取订单项所属的产品ID，使用productService.get方法获取该产品对象
            Product product = productService.get(orderItem.getProductOrderItem_product().getProduct_id());
//            使用categoryService.get方法获取产品的类别对象，并将其设置给产品对象的Product_category属性
            product.setProduct_category(categoryService.get(product.getProduct_category().getCategory_id()));
//            使用productImageService.getList方法获取产品的首长图片
            product.setSingleProductImageList(productImageService.getList(product.getProduct_id(), (byte) 0, new PageUtil(0, 1)));
//            将订单项的价格加到orderTotalPrice变量上
...
//        为订单设置产品订单项列表。获取与订单ID相关的产品订单项列表
...
//        检查订单中的产品订单项列表的大小，如果列表大小为1，表示订单只有一个订单项
...
//            获取第一个订单项，并获取与订单项相关联的产品信息
            Product product = productService.get(productOrderItem.getProductOrderItem_product().getProduct_id());
//            获取产品的分类信息，并将其设置到产品对象中
            product.setProduct_category(categoryService.get(product.getProduct_category().getCategory_id()));
//            设置订单项的产品信息为刚刚获取到的产品对象，并将订单项的价格设置为productOrderItem.setProductOrderItem_product(product)
            productOrderItem.setProductOrderItem_product(product);
            orderTotalPrice = productOrderItem.getProductOrderItem_price();
        } else {
//            如果订单中有多个订单项，代码会遍历订单的产品订单项列表，并将每个订单项的价格累加到orderTotalPrice中
...
//        将orderItemMap转换为JSON对象
        JSONObject orderItemString = JSON.parseObject(orderItemMap);
//        通过orderItemString.keySet方法获取JSONObject对象的键集，表示订单项的ID集合
...
//                从请求参数中获取订单项对应的产品数量，并将其转换为Short类型的数值
...
//                根据订单项的价格和数量计算出单个产品的价格，然后更新订单项的价格和数量
...
//        调用productOrderService.add方法将订单对象添加到数据库
        Boolean yn = productOrderService.add(productOrder);
        if (!yn) {
            throw new RuntimeException();
        }
//        调用lastIDService.selectLastID方法获取最后插入的订单ID
        Integer order_id = lastIDService.selectLastID();
        logger.info("整合订单项对象");
//        声明并初始化一个ProductOrderItem对象
        ProductOrderItem productOrderItem = new ProductOrderItem()
...
//        调用productOrderItemService.add方法将订单项对象添加到数据库中
...
//                调用productOrderItemService.update方法更新订单项的用户留言信息
...
//                通过订单项对象的产品ID获取相应的产品对象，并将其设置为订单项的产品对象
