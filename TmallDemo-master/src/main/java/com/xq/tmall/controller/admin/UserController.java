...
//        创建一个pageUtil对象，并设置起始索引和每页数量
        PageUtil pageUtil = new PageUtil(0, 10);
//        调用userService.getList方法来获取用户列表，并将结果存储在userList变量中
...
//        将addressStack中的地址信息出栈并拼接到builder字符串中。
...
//        调用productOrderItemService.getListByUserId方法，通过用户ID获取购物车订单项列表
...
//                通过productService.get方法，通过ID获取产品对象
                Product product = productService.get(productId);
                if (product != null) {
                    logger.warn("获取产品ID为{}的第一张预览图片信息", productId);
//                    调用productImageService.getList方法获取第一张预览图片，并设置到产品对象中
...
//        设置用户的购物车订单项列表为获取到的订单
    ...
    public String getUserBySearch(@RequestParam(required = false) String user_name/* 用户名称 */,
        ...
        if (user_gender_array != null && user_gender_array.length == 1) {
            gender = user_gender_array[0];//设置gender变量为性别数组的第一个元素
        }
        ...
        //封装查询条件
        //创建一个User对象，并根据前面处理得到的用户姓名和性别赋值给user对象的相应属性
        User user = new User()
                .setUser_name(user_name)
                .setUser_gender(gender);
//        根据条件创建一个OrderUtil对象，用于指定查询结果的排序方式
        OrderUtil orderUtil = null;
        if (orderBy != null) {
            logger.info("根据{}排序，是否倒序:{}", orderBy, isDesc);
            orderUtil = new OrderUtil(orderBy, isDesc);
        }
//        创建一个JSONObject对象，用于存储查询结果
        JSONObject object = new JSONObject();
        logger.info("按条件获取第{}页的{}条用户", index + 1, count);
//        创建一个PageUtil对象，用于分页查询
        PageUtil pageUtil = new PageUtil(index, count);
//        调用userService.getList方法进行用户查询，将查询结果存储在userList中
        List<User> userList = userService.getList(user, orderUtil, pageUtil);
//        将userList转换为JSONArray并存储在object中
        object.put("userList", JSONArray.parseArray(JSON.toJSONString(userList)));
        logger.info("按条件获取用户总数量");
//        调用userService.getTotal方法获取用户总数量，并将其存储在userCount中
