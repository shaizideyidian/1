...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑商家</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>商家管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑商家</span></li>
			</ol>
		</div>
			...
			<form id="addOrUpdateForm">
				...
				<div class="form-item-img">
                    ...
                    <div style="display: flex;align-items: center;">
                        <div v-for="(item,index) in shangjiatupianImgList" :key="index" style="position: relative;">
                            <div style="position: absolute;right: 10px;top: 0;width: 32px;height: 32px;background: #fff;padding: 6px;font-size: 0;cursor: pointer;border-radius: 0 20px 0 0;" @click="shangjiatupianDelClick(index)">
					...
					<div class="upload btn-img">选择文件<input name="file" type="file" id="shangjiatupianupload" class="form-control-file"></div>
					<input name="shangjiatupian" id="shangjiatupian-input" type="hidden">
				</div>
				<div class="form-item">
					<label>商家类型</label>
					<input name="shangjialeixing" id="shangjialeixing" name="shangjialeixing" class="form-control" placeholder="请输入商家类型">
				</div>
				<div class="form-item">
					<label>咨询电话</label>
	...
	<script>
		...
		var vm = new Vue({
		  ...
		  beforeCreate: function(){
			...
			methods: { 
                shangjiatupianDelClick(index){
                    var mymessage = confirm("确定移除图片？");
                    if (mymessage == true) {
                        this.shangjiatupianImgList.splice(index,1)  
                    }
		...
		// 文件上传
		function upload(){
			$('#shangjiatupianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {				
					document.getElementById('shangjiatupian-input').setAttribute('value',"upload/"+data.result.file);
					if(document.getElementById('shangjiatupianFileName') != null){
						document.getElementById('shangjiatupianFileName').innerHTML = "上传成功!";
					}				
                    vm.shangjiatupianImgList.push("upload/"+data.result.file)
				}
			});
		}  
		//取消 
		...
		function submit() {
			...
			if(validform() ==true && compare() == true){
                ...
                if(vm.shangjiatupianImgList.length){
                    data.shangjiatupian = vm.shangjiatupianImgList.join(',')
                }
		...
		function dataBind(){
            ...
            if(ruleForm.shangjiatupian){
                vm.shangjiatupianImgList = ruleForm.shangjiatupian.split(',')
            }
			$("#shangjiatupian-input").val(ruleForm.shangjiatupian);
			$("#shangjialeixing").val(ruleForm.shangjialeixing);	
		...
		//图片显示
		function showImg(){
			var shangjiatupianFileName = "\"" + ruleForm.shangjiatupian + "\"";
			$("#shangjiatupianImg").attr("src",baseUrl+ruleForm.shangjiatupian);
		}		
		//跨表
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.shangjiatupian != null && res.data.shangjiatupian != ''){

                        vm.shangjiatupianImgList = res.data.shangjiatupian.split(',')
					}
					if(res.data != null && res.data != '' && res.data.shangjialeixing != null && res.data.shangjialeixing != ''){

						$("#shangjialeixing").val(res.data.shangjialeixing);
						$("#shangjialeixing").attr('readonly','readonly');
					}
