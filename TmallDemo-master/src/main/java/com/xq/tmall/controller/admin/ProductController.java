...
//        categoryService.getList方法传入null和null作为参数，来获取产品分类列表，并将结果保存到categoryList中
        List<Category> categoryList = categoryService.getList(null, null);
        map.put("categoryList", categoryList);
        logger.info("获取前10条产品列表");
//        创建一个pageUtil对象，设置其页数为0，行数为10
        PageUtil pageUtil = new PageUtil( 0, 10);
//        通过productService.getList方法，传入null，null，null和pageUtil作为参数，获取前10条产品列表，并将结果保存在productList
        List<Product> productList = productService.getList(null, null, null, pageUtil);
        map.put("productList", productList);
        logger.info("获取产品总数量");
//        通过productService.getTotal方法，传入参数null和null，以获取产品总数量，并将结果保存在productCount中
...
//        将SingleProductImageList和DetailProductImageList分别设置到product对象中
...
//        调用propertyValueService.getList方法，传入一个新创建的PropertyValueList对象，设置PropertyValue对象的product属性为product，以获取产品的属性值列表
...
//        调用propertyService.getList方法，传入一个新创建的propertyList对象，设置Property对象的category属性为product的category，以获取与产品分类对应的属性列表，
...
//        遍历属性列表和属性值列表，如果属性的属性ID与属性值的属性的属性ID相等，将属性值添加到一个新创建的列表
        ...
        logger.info("获取分类列表");
//        调用categoryService.getList，传入参数null和null，以获取分类列表
        List<Category> categoryList = categoryService.getList(null,null);
        map.put("categoryList",categoryList);
...
//        获取管理员权限
        ...
        logger.info("获取分类列表");
//        通过categoryService.getList方法，传入null和null参数，以获取分类列表
        List<Category> categoryList = categoryService.getList(null,null);
        map.put("categoryList",categoryList);
        logger.info("获取第一个分类信息对应的属性列表");
//        通过propertyService.getList方法，设置Property_category属性为categoryList中的第一个分类信息
//        传入两个参数，以获取与第一个分类信息对应的属性列表，并将结果保存在propertyList中
...
//        将json字符串propertyJson解析为一个jsonobject对象，这个对象可以通过键值对的方式获取json中的数据
        JSONObject object = JSON.parseObject(propertyJson);
//        通过调用keySet方法，获取jsonObject对象中的所有的键组成的集合，在这段代码中，将获取到的键集合赋值给了propertyIdSet变量
...
//                通过object.getString方法，从jsonObject对象中根据键获取对应的值，将该值保存在value中
                String value = object.getString(key);
//                创建一个属性值对象
                PropertyValue propertyValue = new PropertyValue()
                        .setPropertyValue_value(value)
//                        创建一个属性对象Property，通过setProperty_id方法设置属性ID为key对应的整数值
                        .setPropertyValue_property(new Property().setProperty_id(Integer.valueOf(key)))
//                        创建一个产品对象Product，通过setProduct_id方法设置产品ID为外部传入的product_id
                        .setPropertyValue_product(new Product().setProduct_id(product_id));
                propertyValueList.add(propertyValue);//将属性值对象添加到属性值列表中
            }
...
//        检验产品预览图片列表productSingleImageList和产品详情图片列表是否为空以及长度是否大于0
...
//                创建一个新的产品图片对象ProductImage，并设置其属性，
//                将该产品图片对象添加到产品图片列表中
...
//            调用productImageService.addList方法添加产品图片列表，并复制返回值给变量yn
...
//        如果产品详情图片列表不为空，进入第二个if语句，逻辑与前一个if相似
...
//        调用productService.update方法更新产品信息
...
//        使用JSON.parseObject方法解析属性添加的JSON字符串propertyAddJson，并将解析结果存储在JSONObject对象中
        JSONObject object = JSON.parseObject(propertyAddJson);
//        获取object中的所有key值，即属性ID的集合propertyIdSet
        Set<String> propertyIdSet = object.keySet();
        if (propertyIdSet.size() > 0) {
            logger.info("整合产品子信息-需要添加的产品属性");
//            创建一个空的产品属性值列表propertyValueList
            List<PropertyValue> propertyValueList = new ArrayList<>(5);
//            使用循环遍历属性ID集合中的每个属性ID
            for (String key : propertyIdSet) {
//                从object中根据key获取属性值
                String value = object.getString(key);
//                创建一个propertyValue对象，并设置其属性值，包括属性值，属性，产品
                PropertyValue propertyValue = new PropertyValue()
                        .setPropertyValue_value(value)
                        .setPropertyValue_property(new Property().setProperty_id(Integer.valueOf(key)))
...
//            调用propertyValueService.addList方法添加产品属性值列表
            yn = propertyValueService.addList(propertyValueList);
            if (yn) {
                logger.info("产品属性添加成功！");
            } else {
...
//        使用JSON.parseObject方法解析属性更新的JSON字符串propertyUpdateJson
...
//            使用循环遍历属性ID集合中的每个属性ID
            for (String key : propertyIdSet) {
//                从object中根据key获取属性值（value）
                String value = object.getString(key);
//                创建一个propertyValue对象，并设置其属性值，包括属性值，属性ID
                PropertyValue propertyValue = new PropertyValue()
                        .setPropertyValue_value(value)
...
//                调用propertyValueService.update方法更新该产品属性值，并将返回值赋值给yn
...
//        如果propertyDeleteList不为null且长度大于0
...
//            调用propertyValueService.deleteList方法根据属性ID列表删除产品属性值，并将返回结果赋值给yn
...
//        判断productSingleImageList是否为空且长度大于0
        if (productSingleImageList != null && productSingleImageList.length > 0) {
            logger.info("整合产品子信息-产品预览图片");
//            创建一个空的产品图片列表productImageList
            List<ProductImage> productImageList = new ArrayList<>(5);
//            使用循环遍历productSingleImageList列表中的每个图片名称
            for (String imageName : productSingleImageList) {
//                创建一个ProductImage对象，并设置其属性，包括图片类型，图片源，关联的产品
                productImageList.add(new ProductImage()
                        .setProductImage_type((byte) 0)
                        .setProductImage_src(imageName.substring(imageName.lastIndexOf("/") + 1))
            ...
            logger.info("共有{}条产品预览图片数据", productImageList.size());
//            调用productImageService.addList方法将产品图片列表添加到数据库
            yn = productImageService.addList(productImageList);
            if (yn) {
                logger.info("产品预览图片添加成功！");
            } else {
...
//        判断productDetailsImageList是否为空且长度大于0
        if (productDetailsImageList != null && productDetailsImageList.length > 0) {
            logger.info("整合产品子信息-产品详情图片");
//            创建一个空的产品图片列表productImageList
            List<ProductImage> productImageList = new ArrayList<>(5);
//            循环遍历productDetailsImageList列表中的每个图片名称
            for (String imageName : productDetailsImageList) {
                productImageList.add(new ProductImage()
                        .setProductImage_type((byte) 1)
                        .setProductImage_src(imageName.substring(imageName.lastIndexOf("/") + 1))
...
//        将propertyList转换为JSON数组，并将其放入object中的名为propertyList的键中
...
//        使用subString和lastIndexOf方法截取文件名中的扩展名部分，并将其存储在extension变量中
