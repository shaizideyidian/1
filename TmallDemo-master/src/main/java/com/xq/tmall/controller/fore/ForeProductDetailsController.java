...
//        获取产品的分类信息，调用了categoryService.get方法，根据product对象中的getProduct_category属性中的Category_id来获取
        product.setProduct_category(categoryService.get(product.getProduct_category().getCategory_id()));
        logger.info("获取产品子信息-产品图片信息");
//        用于获取产品的图片信息，并将其根据图片类型进行分类，首先通过productImageService.getList传入参数，获取与该产品相关的图片信息
        List<ProductImage> productImageList = productImageService.getList(product_id, null, null);
//        创建两个List集合用来存储类型为0(单一图片)和非0(详情图片)的产品图片
...
//        将属性列表和属性值列表进行合并。根据他们的ID进行匹配
//        在每次内部循环中，他首先判断当前的属性ID是否和当前属性值的属性ID相等
        for (Property property : propertyList) {
            for (PropertyValue propertyValue : propertyValueList) {
                if (property.getProperty_id().equals(propertyValue.getPropertyValue_property().getProperty_id())) {
//                    如果相等，则创建一个新的只包含当前属性值的属性值列表，并将其添加到当前属性的属性值列表中
