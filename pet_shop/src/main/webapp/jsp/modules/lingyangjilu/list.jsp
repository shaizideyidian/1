...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">领养记录管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>领养记录管理</span></li>
				<li class="breadcrumb-item-two"><span>领养记录列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				<div class="form-item">
					<label>领养单号</label>
					<input type="text" id="lingyangdanhaoSearch" class="form-control form-control-sm" placeholder="请输入领养单号" aria-controls="tableId">
				</div>
				<div class="form-item">
					<label>宠物品种</label>
					<input type="text" id="chongwupinzhongSearch" class="form-control form-control-sm" placeholder="请输入宠物品种" aria-controls="tableId">
				</div>
				<div class="form-item">
					<label>申请日期</label>
					<div class="item-body">
						<input id="shenqingriqi_start" class="begin Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd'})">
						<span>至</span>
						<input id="shenqingriqi_end" class="end Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd'})">
					</div>
				</div>
				<div class="form-item">
					<label>审核状态</label>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('lingyangdanhao')">领养单号<i id="lingyangdanhaoIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#lingyangdanhaoSearch').val() != null && $('#lingyangdanhaoSearch').val() != ''){
				searchForm.lingyangdanhao ="%" +  $('#lingyangdanhaoSearch').val() + "%";
			}
			
			if($('#chongwupinzhongSearch').val() != null && $('#chongwupinzhongSearch').val() != ''){
				searchForm.chongwupinzhong ="%" +  $('#chongwupinzhongSearch').val() + "%";
			}
			
			if($('#shenqingriqi_start').val() != null && $('#shenqingriqi_start').val() != ''){
				searchForm.shenqingriqi_start = $('#shenqingriqi_start').val();
			}
			if($('#shenqingriqi_end').val() != null && $('#shenqingriqi_end').val() != ''){
				searchForm.shenqingriqi_end = $('#shenqingriqi_end').val();
			}
        ...
        function getDataList() {
			http("lingyangjilu/page","GET",{
				page: pageIndex,
				limit: pageSize,
                sfsh: searchForm.sfsh,
				sort: sortColumn,
				order: sortOrder,
				lingyangdanhao : searchForm.lingyangdanhao,	
				chongwupinzhong : searchForm.chongwupinzhong,	
				shenqingriqi_start : searchForm.shenqingriqi_start,
				shenqingriqi_end : searchForm.shenqingriqi_end,
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var lingyangdanhaoCell  = document.createElement('td');

						 lingyangdanhaoCell.innerHTML = item.lingyangdanhao;

			row.appendChild(lingyangdanhaoCell);
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("lingyangjilu/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("lingyangjilu/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("lingyangjilu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function getSum(colName){
		http("lingyangjilu"+colName,"GET",{
			tableName: "lingyangjilu",
			columnName: colName
		},(res)=>{
			if(res.code == 0){
				return res.data.sum;
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"lingyangjilu");
