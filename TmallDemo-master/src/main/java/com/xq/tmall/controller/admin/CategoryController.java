...
//        首先创建一个JSONObject对象，用于构建响应的JSON数据
        JSONObject jsonObject = new JSONObject();
        logger.info("整合分类信息");
//        整合分类信息，创建一个category对象，并设置分类名称和分类图片路径，其中substring和lastIndexOf方法，截取分类图片路径中最后一个/后面的部分作为分类图片的文件名
        Category category = new Category()
                .setCategory_name(category_name)
                .setCategory_image_src(category_image_src.substring(category_image_src.lastIndexOf("/") + 1));
        logger.info("添加分类信息");
//        添加分类信息，并将返回的结果保存在yn变量中，如果添加分类成功，执行下面操作
...
//        首先创建一个JSONObject对象，用于构建响应JSON数据
        JSONObject jsonObject = new JSONObject();
        logger.info("整合分类信息");
//        整合分类信息，创建一个Category对象，并设置分类ID，分类名称和分类图片路径，其中截取图片路径中最后一个/后面的部分作为分类图片的文件名
        Category category = new Category()
                .setCategory_id(category_id)
                .setCategory_name(category_name)
                .setCategory_image_src(category_image_src.substring(category_image_src.lastIndexOf("/") + 1));
        logger.info("更新分类信息，分类ID值为：{}", category_id);
//        调用update方法，更新分类信息，并将返回的结果保存到yn变量中
...
//        创建一个PageUtil对象，并根据传入的页数和行数初始化。调用getList方法，获取符合搜索条件的分类列表，并将结果保存在categoryList变量中
...
//        调用getTotal方法，获取符合搜索条件的分类总数量，并将结果保存在categoryCount变量中
